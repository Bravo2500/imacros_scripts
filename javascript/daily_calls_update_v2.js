...
var hylas = 3;
var csv_file_name = "_desc_open_calls.csv";
var row;
...
function calls_setup (hylas,wrong_queue)
{
	macro = "CODE:";
...
function update_descriptions(eol,hylas,csv_file_name,current_date,text_do_zapisania)
{	
	...
	for (var a=2; a < eol ; a++)
	{
			...
			macro = "CODE:";
			macro += "SET !DATASOURCE "+hylas+csv_file_name+"\n";
			...
			macro += "TAG POS=1 TYPE=TEXTAREA FORM=ID:OSSMeta-form ATTR=ID:edit-reason CONTENT=\"Description Updated.\"\n";
...
calls_setup(1,wrong_queue);
save_calls(1,csv_file_name);
save_calls(3,csv_file_name);
macro_run(macro);
h1_eol = count_rows(datasource,1,csv_file_name);
h2_eol = count_rows(datasource,3,csv_file_name);
update_descriptions(h1_eol,1,csv_file_name,current_date,text_do_zapisania);
update_descriptions(h2_eol,3,csv_file_name,current_date,text_do_zapisania);
